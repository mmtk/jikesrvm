...
public class AbstractBarriers {
  ...
  private static final boolean NEEDS_BOOLEAN_GC_WRITE_BARRIER     = false; // Selected.Constraints.get().needsBooleanWriteBarrier();
  ...
  private static final boolean NEEDS_BOOLEAN_GC_READ_BARRIER      = false; // Selected.Constraints.get().needsBooleanReadBarrier();
  ...
  public static final boolean BOOLEAN_BULK_COPY_SUPPORTED         = false; // !(NEEDS_BOOLEAN_ASTORE_BARRIER || NEEDS_BOOLEAN_ALOAD_BARRIER) || Selected.Constraints.get().booleanBulkCopySupported();
