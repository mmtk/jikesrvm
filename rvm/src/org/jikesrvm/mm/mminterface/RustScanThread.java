...
@Uninterruptible public final class RustScanThread {
  ...
  private boolean setUpFrame(int verbosity) {
    ...
    if (compiledMethodId == StackFrameLayout.getInvisibleMethodID()) {
      if (fp.minus(8).loadInt() == 42) { // FIXME extremely hacky way to identify glue frames
        if (verbosity != 0) {
          VM.sysWriteln("Process glue frame");
        }
        AddressArray rl = this.iteratorGroup.getRegisterLocations();
        // EBP, EDI, EBX
        rl.set(RegisterConstants.EBX.value(), this.fp.minus(0xc));
        rl.set(RegisterConstants.EDI.value(), this.fp.minus(0x10));
        rl.set(RegisterConstants.EBP.value(), this.fp.minus(0x14));
        if (verbosity != 0) {
          VM.sysWrite("UpdateRegisterLocations: Register ", RegisterConstants.EBX.value());
          VM.sysWriteln(" to Location ", this.fp.minus(0xc));
          VM.sysWrite("UpdateRegisterLocations: Register ", RegisterConstants.EDI.value());
          VM.sysWriteln(" to Location ", this.fp.minus(0x10));
          VM.sysWrite("UpdateRegisterLocations: Register ", RegisterConstants.EBP.value());
          VM.sysWriteln(" to Location ", this.fp.minus(0x14));
        }
      }
      return false;
    }
